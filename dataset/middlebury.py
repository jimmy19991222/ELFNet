...
class MiddleburyBaseDataset(data.Dataset):
    ...
    def _read_data(self):
        self.left_data = [os.path.join(obj, self.left_fname)
                          for obj in os.listdir(os.path.join(self.datadir))]
        self.right_data = [os.path.join(obj, self.right_fname)
                           for obj in os.listdir(os.path.join(self.datadir))]
    ...
    def __getitem__(self, idx):
        ...
        if not self.split == 'test':  # no disp for test files
            # occ
            occ_left_fname = os.path.join(
                self.datadir, self.occ_left_data[idx])
            occ_right_fname = os.path.join(
                self.datadir, self.occ_right_data[idx])
            occ_left = np.array(Image.open(occ_left_fname)) == 128
            occ_right = np.array(Image.open(occ_right_fname)) == 128

            # disp
            disp_left_fname = os.path.join(
                self.datadir, self.disp_left_data[idx])
            disp_right_fname = os.path.join(
                self.datadir, self.disp_right_data[idx])

            disp_left, _ = readPFM(disp_left_fname)
            disp_right, _ = readPFM(disp_right_fname)

            # occ_left = np.ones(disp_left.shape) != 1
            # occ_right = np.ones(disp_left.shape) != 1
