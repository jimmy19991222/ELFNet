...
class KITTIBaseDataset(data.Dataset):
    ...
    def _read_data(self):
        ...
        self.left_data = natsorted([os.path.join(self.datadir, self.sub_folder, self.left_fold, img) for img in
                                    ...
                                    img.find('_10') > -1])
        self.right_data = [img.replace(
            self.left_fold, self.right_fold) for img in self.left_data]
        self.disp_data = [img.replace(
            self.left_fold, self.disp_fold) for img in self.left_data]
    ...
    def _split_data(self):
        ...
        if len(self.left_data) > 1:
            if self.split == 'train':
                self.left_data = self.left_data[:int(
                    len(self.left_data) * train_val_frac)]
                self.right_data = self.right_data[:int(
                    len(self.right_data) * train_val_frac)]
                self.disp_data = self.disp_data[:int(
                    len(self.disp_data) * train_val_frac)]
...
class KITTI2015Dataset(KITTIBaseDataset):
    def __init__(self, datadir, split='train'):
        ...
        # we read disp data with occlusion since we compute occ directly
...
class KITTIDataset(KITTIBaseDataset):
    ...
    def __init__(self, datadir, split='train'):
        ...
        # we we read disp data with occlusion since we compute occ directly
    ...
    def _read_data(self):
        ...
        right_data_2015 = [img.replace(
            self.left_fold_2015, self.right_fold_2015) for img in left_data_2015]
        disp_data_2015 = [img.replace(
            self.left_fold_2015, self.disp_fold_2015) for img in left_data_2015]
        ...
        right_data_2012 = [img.replace(
            self.left_fold_2012, self.right_fold_2012) for img in left_data_2012]
        disp_data_2012 = [img.replace(
            self.left_fold_2012, self.disp_fold_2012) for img in left_data_2012]
