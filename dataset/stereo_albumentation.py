...
def crop(img, x1, y1, x2, y2):
    ...
    if (len(img.shape) == 3):
        ...
        print("size {} is not legal!".format(img.shape))
...
def random_crop(min_crop_height, min_crop_width, input_data, split):
    ...
    if min_crop_height >= height or min_crop_width > width:
        ...
        x1, y1, x2, y2 = width - crop_width, height - crop_height, width, height
    ...
    try:
        input_data['disp_right'] = crop(
            input_data['disp_right'], x1, y1, x2, y2)
...
def simple_crop(min_crop_height, min_crop_width, input_data, split):
    """
    Crop center part of the input with a random width and height.
...
def resize_all(h, w, input_data):
    # aug = Resize([h, w])
    # print(input_data['left'].shape)
    input_data['left'] = resize(torch.tensor(
        input_data['left'].copy()), [h, w])
    input_data['right'] = resize(torch.tensor(
        input_data['right'].copy()), [h, w])
    input_data['disp'] = resize(torch.tensor(
        input_data['disp'].copy()), [h, w])
    input_data['occ_mask'] = resize(torch.tensor(
        input_data['occ_mask'].copy()), [h, w])
    try:
        input_data['disp_right'] = resize(torch.tensor(
            input_data['disp_right'].copy()), [h, w])
        input_data['occ_mask_right'] = resize(torch.tensor(
            input_data['occ_mask_right'].copy()), [h, w])
    except KeyError:
        pass
...
class RGBShiftStereo(StereoTransformAsym, RGBShift):
    ...
    def __init__(self, r_shift_limit=20, g_shift_limit=20, b_shift_limit=20, always_apply=False, p=0.5, p_asym=0.2):
        StereoTransformAsym.__init__(self, always_apply, p, p_asym)
        RGBShift.__init__(self, r_shift_limit, g_shift_limit,
                          b_shift_limit, always_apply, p)
    ...
    def get_params_dependent_on_targets(self, params):
        r_shift_l = random.uniform(
            self.r_shift_limit[0], self.r_shift_limit[1])
        g_shift_l = random.uniform(
            self.g_shift_limit[0], self.g_shift_limit[1])
        b_shift_l = random.uniform(
            self.b_shift_limit[0], self.b_shift_limit[1])

        if self.asym():
            r_shift_r = random.uniform(
                self.r_shift_limit[0], self.r_shift_limit[1])
            g_shift_r = random.uniform(
                self.g_shift_limit[0], self.g_shift_limit[1])
            b_shift_r = random.uniform(
                self.b_shift_limit[0], self.b_shift_limit[1])
        else:
            r_shift_r = r_shift_l
            g_shift_r = g_shift_l
