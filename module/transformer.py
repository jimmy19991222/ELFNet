...
class Transformer(nn.Module):
    ...
    def _alternating_attn(self, feat: torch.Tensor, pos_enc: torch.Tensor, pos_indexes: Tensor, hn: int):
        ...
        for idx, (self_attn, cross_attn) in enumerate(zip(self.self_attn_layers, self.cross_attn_layers)):
            ...
            feat = checkpoint(create_custom_self_attn(
                self_attn), feat, pos_enc, pos_indexes)
    ...
    def forward(self, feat_left: torch.Tensor, feat_right: torch.Tensor, pos_enc: Optional[Tensor] = None):
        ...
        feat_left = feat_left.permute(1, 3, 2, 0).flatten(
            2).permute(1, 2, 0)  # CxWxHxN -> CxWxHN -> WxHNxC

        feat_right = feat_right.permute(1, 3, 2, 0).flatten(2).permute(1, 2, 0)
        if pos_enc is not None:
            with torch.no_grad():
                # indexes to shift rel pos encoding
                indexes_r = torch.linspace(
                    w - 1, 0, w).view(w, 1).to(feat_left.device)
                indexes_c = torch.linspace(
                    0, w - 1, w).view(1, w).to(feat_left.device)
                # WxW' -> WW'
        ...
        # compute attention
        attn_weight, feat = self._alternating_attn(
            feat, pos_enc, pos_indexes, hn*bs)
        attn_weight = attn_weight.view(hn, bs, w, w).permute(
            1, 0, 2, 3)  # NxHxWxW, dim=2 left image, dim=3 right image
...
class TransformerCrossAttnLayer(nn.Module):
    ...
    def forward(self, feat_left: Tensor, feat_right: Tensor,
        ...
        if last_layer:
            ...
            attn_mask = self._generate_square_subsequent_mask(
                w).to(feat_left.device)  # generate attn mask
        else:
            attn_mask = None
