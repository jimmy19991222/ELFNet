...
class RegressionHead(nn.Module):
    ...
    def _upsample(self, x: NestedTensor, disp_pred: Tensor, occ_pred: Tensor, scale: int):
        ...
        disp_pred = F.interpolate(disp_pred_attn[None,], size=(h, w), mode='nearest').transpose(0,1)  # N x 1 x H x W
        occ_pred = F.interpolate(occ_pred[None,], size=(h, w), mode='nearest').transpose(0,1)  # N x 1 x H x W
        ...
        if self.cal is not None:
            ...
            disp_pred_normalized, occ_pred = self.cal(disp_pred_normalized, occ_pred_normalized, x.left.cuda())  # N x H x W
    ...
    def forward(self, attn_weight: Tensor, x: NestedTensor):
        ...
        if x.disp is not None:
            ...
            output['gt_response'], target = self._compute_gt_location(scale, x.sampled_cols.cuda(), x.sampled_rows.cuda(),
                                                                      attn_ot[..., :-1, :-1].cuda(), x.disp.cuda())
        else:
        ...
        if x.occ_mask is not None:
            ...
            output['gt_response_occ_left'] = attn_ot[..., :-1, -1][occ_mask.cuda()]
            output['gt_response_occ_right'] = attn_ot[..., -1, :-1][occ_mask_right.cuda()]
        else:
        ...
        output['occ_pred_low_res'] = occ_pred_low_res
